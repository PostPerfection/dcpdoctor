...
static CtpTestResult make_ctp(const std::string& id, CtpCategory cat,
                              const std::string& desc, const std::string& req,
...
static CtpTestResult make_skipped(const std::string& id, CtpCategory cat,
                                  const std::string& desc, const std::string& req,
...
CtpResult run_ctp_tests(const CtpOptions& opts)
{
  ...
  if(opts.test_packaging)
  {
    ...
    result.results.push_back(
        make_ctp("CTP-PKG-001", CtpCategory::Packaging,
    ...
    result.results.push_back(
        make_ctp("CTP-PKG-002", CtpCategory::Packaging,
    ...
    result.results.push_back(
        make_ctp("CTP-PKG-003", CtpCategory::Packaging,
    ...
    result.results.push_back(
        make_ctp("CTP-PKG-004", CtpCategory::Packaging,
  ...
  if(opts.test_composition)
  {
    ...
    result.results.push_back(
        make_ctp("CTP-CPL-001", CtpCategory::Composition,
    ...
    if(first_cpl.has_value())
    {
      ...
      result.results.push_back(
          make_ctp("CTP-CPL-004", CtpCategory::Composition,
      ...
      static const std::set<std::string> valid_rates = {
          "24 1", "25 1", "30 1", "48 1", "50 1", "60 1"};
      bool valid_rate = true;
      for(const auto& reel : first_cpl->reels)
      {
        if(!reel.picture.edit_rate.empty() &&
           !valid_rates.contains(reel.picture.edit_rate))
        {
          valid_rate = false;
          break;
        }
      ...
      result.results.push_back(
          make_ctp("CTP-CPL-005", CtpCategory::Composition,
  ...
  if(opts.test_picture)
  {
    ...
    result.results.push_back(
        make_skipped("CTP-PIC-001", CtpCategory::Picture,
    ...
    for(const auto& entry : fs::directory_iterator(opts.dcp_dir))
    {
      ...
      {
        result.results.push_back(
            make_ctp("CTP-PIC-004", CtpCategory::Picture,
  ...
  {
    result.results.push_back(
        make_skipped("CTP-AUD-001", CtpCategory::Audio,
  ...
  if(opts.test_security)
  {
    ...
    result.results.push_back(
        make_ctp("CTP-SEC-001", CtpCategory::Security,
                 "Encryption status",
  ...
  {
    result.results.push_back(
        make_skipped("CTP-PRE-001", CtpCategory::Presentation,
  ...
  spdlog::info("DCI CTP: {}/{} passed, {} skipped, {} failed ({})",
