...
- Bv2.1 timed-text content limits (line length, line count, cue duration and spacing, caption charset) run on `MainSubtitle` tracks and on caption tracks named either `MainClosedCaption` (what the digicine CC-CPL schema declares and real Bv2.1 packages ship) or `ClosedCaption`, with the stricter caption limits on the latter two.
